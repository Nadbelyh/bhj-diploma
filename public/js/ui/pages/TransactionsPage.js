...
class TransactionsPage {
  ...
  render(options) {
    if (options) {
      ...
      this.currentAccName; // Для красоты - в методе removeAccount подставим в confirm название счета
  ...
  formatDate(date) {
    ...
    const returnDateFormat = `${dateFormat.getDate()} ${
      month[dateFormat.getMonth()]
    } ${dateFormat.getFullYear()} г. в ${dateFormat.getHours()}:${dateFormat.getMinutes()}`;
